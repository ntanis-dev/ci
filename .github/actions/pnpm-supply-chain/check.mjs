...
for (const path of ['pnpm-lock.yaml', 'pnpm-workspace.yaml', '.npmrc', 'renovate.json', 'SECURITY.md', '.github/CODEOWNERS', '.github/workflows/osv-scanner.yml']) {
...
for (const policy of ['minimumReleaseAge: 20160', 'minimumReleaseAgeIgnoreMissingTime: false', 'minimumReleaseAgeStrict: true', 'trustLockfile: false', 'blockExoticSubdeps: true', 'saveExact: true', 'strictDepBuilds: true', 'verifyDepsBeforeRun: error']) {
  if (!pnpmWorkspace.includes(policy)) failures.push(`pnpm-workspace.yaml must contain ${policy}`)
}

const overrides = pnpmWorkspace.match(/^overrides:\s*$([\s\S]*?)(?=^\S|(?![\s\S]))/m)?.[1] ?? ''
for (const line of overrides.split(/\r?\n/)) {
  if (!line.trim() || line.trimStart().startsWith('#')) continue
  const match = /^  (?:'[^']+'|[^:\s]+):\s*(?:'([^']+)'|([^\s]+))$/.exec(line)
  const target = match?.[1] ?? match?.[2]
  if (!target || !exactRegistryTarget(target)) failures.push(`pnpm-workspace.yaml: override target must be exact, found ${line.trim()}`)
}

const allowBuilds = pnpmWorkspace.match(/^allowBuilds:\s*$([\s\S]*?)(?=^\S|(?![\s\S]))/m)?.[1] ?? ''
for (const line of allowBuilds.split(/\r?\n/)) {
  ...
  else if (!exactBuildSelector(match[1])) failures.push(`pnpm-workspace.yaml: build approval must be exact, found ${match[1]}`)
}

const exclusions = pnpmWorkspace.match(/^minimumReleaseAgeExclude:\s*$([\s\S]*?)(?=^\S|(?![\s\S]))/m)?.[1] ?? ''
...
if (!osv.includes('fail-on-vuln: true') && !osv.includes('--fail-on-vuln=true')) failures.push('OSV must fail closed')

if (failures.length) {
  console.error(failures.join('\n'))
  process.exit(1)
}
console.log('ntanis.dev pnpm supply-chain policy is enforced by the pinned central action.')
