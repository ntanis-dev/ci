...
Each matrix entry still declares `install` for backward-compatible schema validation, but every build runner performs `pnpm install --frozen-lockfile`. This is required by fail-closed `verifyDepsBeforeRun` and ensures a source-packaging script cannot execute against an unverified or absent dependency graph. The separate metadata job independently installs and audits the same lockfile before any matrix build proceeds.
